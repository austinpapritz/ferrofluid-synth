...
export class Sketch {
  ...
  camera = {
    ...
    fov: Math.PI / 4,
    aspect: 1,
    position: vec3.fromValues(0, 0.5, 1),
  ...
  #init() {
    ...
    this.integratePrg = twgl.createProgramInfo(gl, [
      integrateVert,
      integrateFrag,
    ]);
    ...
    this.heightMapPrg = twgl.createProgramInfo(gl, [
      heightMapVert,
      heightMapFrag,
    ]);
    ...
    this.pointerParamsUBO = twgl.createUniformBlockInfo(
      gl,
      this.integratePrg,
      "u_PointerParams"
    );
    ...
    this.quadVAO = twgl.createVAOAndSetAttributes(
      gl,
      this.pressurePrg.attribSetters,
      this.quadBufferInfo.attribs,
      this.quadBufferInfo.indices
    );
    const spikesArrays = twgl.primitives.createPlaneVertices(
      1,
      1,
    ...
    this.groundVAO = twgl.createVAOAndSetAttributes(
      gl,
      this.groundPrg.attribSetters,
      this.groundBufferInfo.attribs,
      this.groundBufferInfo.indices
    );
    this.groundWorldMatrix = mat4.create();

    // Setup Framebuffers
    this.pressureFBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.densityPressure }],
      this.textureSize,
      this.textureSize
    );
    this.forceFBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.force }],
      this.textureSize,
      this.textureSize
    );
    this.inFBO = twgl.createFramebufferInfo(
      gl,
      [
        { attachment: this.textures.position1 },
        { attachment: this.textures.velocity1 },
      ],
      this.textureSize,
      this.textureSize
    );
    this.outFBO = twgl.createFramebufferInfo(
      gl,
      [
        { attachment: this.textures.position2 },
        { attachment: this.textures.velocity2 },
      ],
      this.textureSize,
      this.textureSize
    );
    this.indices1FBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.indices1 }],
      this.textureSize,
      this.textureSize
    );
    this.indices2FBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.indices2 }],
      this.textureSize,
      this.textureSize
    );
    this.offsetFBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.offset }],
      this.cellSideCount,
      this.cellSideCount
    );
    this.heightMapFBO = twgl.createFramebufferInfo(
      gl,
      [{ attachment: this.textures.heightMap }],
      this.heightMapSize,
      this.heightMapSize
    );
  ...
  #initEvents() {
    ...
    fromEvent(this.canvas, "pointermove")
      .pipe(filter(() => this.isPointerDown))
      .subscribe((e) => {
        this.pointer = this.#getNormalizedPointerCoords(e);
        this.pointer = this.#getPointerSpikesPlaneIntersection();
      });
  ...
  #initTextures() {
    ...
    this.totalSortSteps =
      ((this.logNumParticles + this.logNumParticles) *
        (this.logNumParticles + this.logNumParticles + 1)) /
      2;
  ...
  #initEnvMap() {
    ...
    this.envMapTexture = twgl.createTexture(
      gl,
      {
  ...
  #updatePointer() {
    ...
    vec2.subtract(
      this.pointerLerpDelta,
      this.pointerLerp,
      this.pointerLerpPrev
    );
    vec2.copy(this.pointerLerpPrev, this.pointerLerp);
  }
  ...
  #simulate(deltaTime) {
    ...
    twgl.setBlockUniforms(this.pointerParamsUBO, {
  ...
  #animate(deltaTime) {
    ...
    if (this.isEntryAnimationDone) {
      ...
      if (this.audioControl.isInitialized)
      ...
      if (this.entryProgress >= this.entryDelay) {
        ...
        const part1 = this.entryDuration * 0.7;
  ...
  #render() {
    ...
    gl.drawElements(
      gl.TRIANGLES,
      this.groundBufferInfo.numElements,
      gl.UNSIGNED_SHORT,
      0
    );
  ...
  #getPointerSpikesPlaneIntersection() {
    ...
    vec3.scale(
      i,
      i,
      1 / (this.ZOOM + this.#remapHeightMapZoomScale(this.ZOOM))
    );
  ...
  #screenToWorldPosition(x, y, z) {
    ...
    const worldPos = vec4.transformMat4(
      vec4.create(),
      vec4.fromValues(ndcPos[0], ndcPos[1], ndcPos[2], 1),
      this.camera.matrices.inversViewProjection
    );
    if (worldPos[3] !== 0) {
  ...
  #remapSmoothFactorZoom(zoom) {
    return 0.3 * zoom * zoom + -0.07 * zoom + 0.02;
  }
